...
type ProSavedFlightLabData = SavedFlightCardData & {
  completed_at?: string | null
}

type ProSavedRoutesLabProps = {
  savedFlights: ProSavedFlightLabData[]
  onOpenSavedFlightIntelligence?: (flight: ProSavedFlightLabData) => void
  onMarkSavedFlightCompleted?: (flight: ProSavedFlightLabData) => void
  onDeleteSavedFlight?: (flight: ProSavedFlightLabData) => void
}
        ...
        {savedFlights.length === 0 ? (
          <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50/70 px-4 py-6">
            ...
            <p className="mt-1 text-sm leading-6 text-slate-600">
              Open route intelligence above and save a flight to track it here.
            </p>
          </div>
        ) : (
          <div className="mt-5 max-h-[430px] space-y-2 overflow-y-auto pr-2 [scrollbar-color:rgba(148,163,184,0.45)_rgba(241,245,249,0.9)] [scrollbar-width:thin] [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-track]:bg-slate-100 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-slate-300 [&::-webkit-scrollbar-thumb:hover]:bg-slate-400">
            {savedFlights.map((flight) => {
              const isOpen = openSavedFlightId === flight.id
...
function getRouteLabel(flight: ProSavedFlightLabData) {
...
function getAirlineLabel(flight: ProSavedFlightLabData) {
  return flight.airline?.trim() || "Saved flight"
}

function getFlightNumberLabel(flight: ProSavedFlightLabData) {
  return flight.flight_number?.trim() || "Flight"
}

function getDepartureLabel(flight: ProSavedFlightLabData) {
...
function getStatusLabel(flight: ProSavedFlightLabData) {
  if (flight.completed_at) return "Completed"
  return "Active"
}

function getSignalLabel(flight: ProSavedFlightLabData) {
  if (flight.completed_at) return "Completed"
